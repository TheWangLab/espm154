...
```{r cand snps}
# We will test for significance at a p-value threshold of 0.01, to be more stringent, but you can adjust this threshold
...
1. Convert the vcf to a dosage matrix (use the `vcf_to_dosage()` and `simple_impute()` functions)
2. Extract the environmental variables at the sampling coordinates (use the `extract()` and `scale()` functions)
...
```{r}
# Convert from vcf to dosage matrix
van_gen <- vcf_to_dosage(van_vcf)

# Running RDA requires that your genotype matrix contains no missing values. We will perform a simple imputation based on the median.
van_gen <- simple_impute(van_gen)

# Extract environmental vars
van_env <- raster::extract(bio, van_coords, ID = FALSE)

# Standardize environmental variables and make into dataframe
van_env <- scale(van_env, center = TRUE, scale = TRUE)

# Run RDA
van_mod <- rda_run(van_gen, van_env)

# Identify outlier SNPs
van_outliers <- rda_getoutliers(van_mod, sig = 0.05, plot = FALSE)

# Visualize results
rda_plot(van_mod, van_outliers$rda_snps, biplot_axes = c(1, 2), rdaplot = TRUE, manhattan = FALSE)

# Identify environmental associations
van_snps <- van_outliers$rda_snps

# Extract genotypes for outlier SNPs
rda_gen <- van_gen[, van_snps]

# Run correlation test
van_cor <- rda_cor(rda_gen, van_env)

# Make a table from these results, ordered by the strength of the correlation:
rda_table(van_cor, order = TRUE, nrow = 10)
```
---------------------------------------------
...
overlapping_snps <- intersect(vos_outliers$rda_snps, van_outliers$rda_snps)

# Calculate how many SNPs are shared between the two species
length(overlapping_snps)

# The setdiff() function will return the SNPs that are present in one species but not the other
nonoverlapping_snps <- setdiff(vos_outliers$rda_snps, van_outliers$rda_snps)

# Calculate how many SNPs are unique to each species
length(nonoverlapping_snps)
```
